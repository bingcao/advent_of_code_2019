use intcode::{process_program, Program};
use permutohedron::Heap;

const NUM_AMPS: usize = 5;

fn get_signal_for_inputs(program_str: &str, inputs: &[i32; 5]) -> i32 {
    let mut programs: Vec<Program> = (0..NUM_AMPS).map(|i| Program::new(program_str, &vec![inputs[i]])).collect();
    let mut output = 0;
    for &input in inputs {
        let (_p, new_output) = process_program(program_str, &[input, output]);
        output = new_output.unwrap();
        println!("Input {} gave output {}", input, output); 
    }
    output
}
...
pub fn get_signal(program_str: &str) -> i32 {
    ...
    permutations.map(|permutation| get_signal_for_inputs(program_str, &permutation)).max().unwrap()
}
